<template>
  <div class="day_inspect_list-warp">
    <h3>日检任务</h3>
    <div class="day_inspect_list-sreach">
    ...
    <div class="day_inspect_list-list">
      <div class="day_inspect_list-item" v-for="item in dayInspectList"  @click="handleItem(item)">
        <h3>{{ item.store_no }}</h3>
        <p>{{ item.store_name }}</p>
      </div>
    </div>
  ...
  methods: {
    ...
    init(){
      ...
      getDayInspectList(obj).then(res => {
        ...
        const zone = { store_id : 0 , ptype : '1' + this.curentKitchen + '2',  store_name :'厨房公共区域'}
        const dbody = data.data || [];
        dbody.forEach( (element, index) => {
          element.ptype = '1' +'' + element.kitchen_id +'' + '1';
        });
        dbody.unshift(zone)
...
<style scoped lang="less">
  ...
  .day_inspect_list-sreach{
    width: 80%;
    text-align: center;
    margin: 1rem auto;
  }
  .day_inspect_list-list{
    width: 100%;
    display: flex;
    justify-content: space-around;
    flex-wrap: wrap;
    .day_inspect_list-item{
      width: 30%;
