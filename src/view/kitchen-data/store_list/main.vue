<template>
  <div>
      ...
      <Card shadow>
        <Row :gutter="20">
          <i-col :xs="4" :md="4" :lg="4">
            <Input v-model="kitchen_name" readonly style="width: 200px"></Input>
          </i-col>
        </Row>
      </Card>
    </div>
    <Tabs :animated="false" style="margin-top: 5px;">
        <TabPane label="起租中">
          <tables ref="start_shops_tables"  v-model="start_shops_list" :columns="start_shops_columns" @data-view="showShopCard"/>
        ...
        <TabPane label="租赁中">
          <tables ref="normal_shops_tables"  v-model="normal_shops_list" :columns="normal_shops_columns" @data-view="showShopCard"/>
        ...
        <TabPane label="退租中">
          <tables ref="end_shops_tables"  v-model="end_shops_list" :columns="end_shops_columns" @data-view="showShopCard"/>
          <Page :total="end_shops_page.total" :page-size="end_shops_page.list_rows" style="margin-top:10px;" @on-change="getEndShopsNewPage"/>
        </TabPane>
        <TabPane label="已归档">
          <tables ref="eliminate_shops_tables"  v-model="eliminate_shops_list" :columns="eliminate_shops_columns" @data-view="showShopCard"/>
...
export default {
  ...
  mounted () {
    let that = this;
    this.select_kitchen_id = this.$route.query.kitchen_id
    getKitchenList().then(res => {
      ...
      if(this.kitchenList.length > 0){
        this.kitchenList.forEach( function(element, index) {
          if( element.id  == that.select_kitchen_id ){
            that.kitchen_name = element.kitchen_name
            that.initData(that.select_kitchen_id);
          }
        });
      }
    })   
